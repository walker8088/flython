...
class IMUCal(QMainWindow, Ui_FreeIMUCal):
  def __init__(self):
    ...
    self.remoteIPEdit.setText(self.settings.value("calgui/remoteIPEdit", "").toString())
    
    # when user hits enter, we generate the clicked signal to the button so that connection starts
    self.connect(self.remoteIPEdit, SIGNAL("returnPressed()"), self.connectButton, SIGNAL("clicked()"))
    
    # Connect up the buttons to their functions
    self.connectButton.clicked.connect(self.remote_connect)
    ...
    self.acc3D.opts['distance'] = 3000
    self.acc3D.show()
    
    self.magn3D.opts['distance'] = 200
    self.magn3D.show()
    
    ax = gl.GLAxisItem()
    ax.setSize(x=2000, y=2000, z=2000)
    self.acc3D.addItem(ax)
    
    mx = gl.GLAxisItem()
    mx.setSize(x=100, y=100, z=100)
  ...
  def remote_connect(self):
    self.remote_ip = str(self.remoteIPEdit.text())
    # save serial value to user settings
    self.settings.setValue("calgui/remoteIPEdit", self.remote_ip)
    
    self.connectButton.setEnabled(False)
    # waiting mouse cursor
    QApplication.setOverrideCursor(QCursor(Qt.WaitCursor))
    self.set_status("Connecting to " + self.remote_ip + " ...")
    
    # TODO: serial port field input validation!
    
    try:
        self.conn = rpyc.connect(self.remote_ip, 5678)
        self.conn.root.init()
        ...
        self.connectButton.clicked.connect(self.remote_disconnect)
        self.remoteIPEdit.setEnabled(False)
  ...
  def remote_disconnect(self):
    print "Disconnecting from " + self.remote_ip
    self.conn.close()
    self.set_status("Disconnected")
    self.remoteIPEdit.setEnabled(True)
    
    self.connectButton.setText("Connect")
    self.connectButton.clicked.disconnect(self.remote_disconnect)
    self.connectButton.clicked.connect(self.remote_connect)
  ...
  def sampling_start(self):
    self.connWorker = ConnWorker(conn = self.conn)
    self.connect(self.connWorker, SIGNAL("new_data(PyQt_PyObject)"), self.newData)
  ...
  def sampling_end(self):
    self.connWorker.exiting = True
    self.connWorker.quit()
    self.connWorker.wait()
...
"""
  ...
  def save_calibration_eeprom(self):
    ...
    #self.conn.write(offsets)
    #self.conn.write(scales)
