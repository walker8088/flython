...
class MPU6050:
    ...
    def getSlaveDelayEnabled(self, num):
        ...
        return self.i2c.readBit(self.addr, MPU6050_RA_I2C_MST_DELAY_CTRL, num)    
    ...
    def getFIFOByte(self):
        return self.i2c.readU8(self.addr, MPU6050_RA_FIFO_R_W)
    
    def getFIFOBytes(self,length):
        return self.i2c.readBytes(self.addr, MPU6050_RA_FIFO_R_W, length)
    ...
    def setXFineGain(self, gain):
        self.i2c.write8(self.addr, MPU6050_RA_X_FINE_GAIN, gain)
        
    def getYFineGain(self):
        return self.i2c.readU8(self.addr, MPU6050_RA_Y_FINE_GAIN)
     
    def setYFineGain(self, gain):
        self.i2c.write8(self.addr, MPU6050_RA_Y_FINE_GAIN, gain)
     
    def getZFineGain(self):
        return self.i2c.readU8(self.addr, MPU6050_RA_Z_FINE_GAIN)
    ...
    def setYGyroOffsetUser(self, value):
        self.i2c.write8(self.addr, MPU6050_RA_YG_OFFS_USRH, value >> 8)
        self.i2c.write8(self.addr, MPU6050_RA_YG_OFFS_USRL, value & 0xFF) 
        return True        
    ...
    def setZGyroOffsetUser(self, value):
        self.i2c.write8(self.addr, MPU6050_RA_ZG_OFFS_USRH, value >> 8)
        self.i2c.write8(self.addr, MPU6050_RA_ZG_OFFS_USRL, value & 0xFF) 
        return True        
    ...
    def setMemoryBank(self, bank, prefetchEnabled = False, userBank = False):
        ...
        self.i2c.write8(self.addr, MPU6050_RA_BANK_SEL, bank)
        return True        
        
    def setMemoryStartAddress(self, address):
        self.i2c.write8(self.addr, MPU6050_RA_MEM_START_ADDR, address)       
        
    def readMemoryByte(self):
        result = self.i2c.readU8(self.addr, MPU6050_RA_MEM_R_W)
        return result

    def writeMemoryByte(self, data):
        self.i2c.write8(self.addr, MPU6050_RA_MEM_R_W, data)
    ...
    def writeMemoryBlock(self, data, dataSize, bank = 0, address = 0, verify = False):
        ...
        i = 0
        while i < dataSize:  
            self.i2c.write8(self.addr, MPU6050_RA_MEM_R_W, data[i])
        ...
        while pos < dataSize:
            ...
            if dmpConfSet[2] > 0:
                ...
                if dmpConfSet[3] == 0x01:
                    ...
                    self.i2c.write8(self.addr, MPU6050_RA_INT_ENABLE, 0x32);  # single operation  

        
    def getDMPConfig1(self):
        self.i2c.readU8(self.addr, MPU6050_RA_DMP_CFG_1)
        
    def setDMPConfig1(self, config):
        self.i2c.write8(self.addr, MPU6050_RA_DMP_CFG_1, config)        
        
    def getDMPConfig2(self):
        return self.i2c.readU8(self.addr, MPU6050_RA_DMP_CFG_2)

    def setDMPConfig2(self, config):
        self.i2c.write8(self.addr, MPU6050_RA_DMP_CFG_2, config)
