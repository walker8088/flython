...
class MPU6050(object):
    ...
    def init(self):    
        ...
        self.i2c.write_mem_byte(self.address, MPU6050_PWR_MGMT_1, 0)
        # Set the gryo resolution
        self.i2c.write_mem_byte(self.address, MPU6050_FS_SEL, self.fs_scale << 3)
        # Set the accelerometer resolution
        self.i2c.write_mem_byte(self.address, MPU6050_AFS_SEL, self.afs_scale << 3)
        #TODO bypass mode
	self.chip_id = self.i2c.read_mem_byte(self.address, 0x75)
	
    def read(self):
        '''
        Read the raw data from the sensor, scale it appropriately and store for later use
        '''
        raw_gyro_data = self.i2c.read_mem_block(self.address, MPU6050_GYRO_START_BLOCK, 6)
        raw_accel_data = self.i2c.read_mem_block(self.address, MPU6050_ACCEL_START_BLOCK, 6)
        raw_temp_data = self.i2c.read_mem_block(self.address, MPU6050_TEMP_START_BLOCK, 2)
    ...
    def update(self):
        self.read()
        return (self.gyro_xyz(), self.accel_xyz())