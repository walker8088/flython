...
class HMC5883L(object):
    ...
    def init(self):
        ...
        self.i2c.write_mem_byte(0x68, 0x6A, 0);
        self.i2c.write_mem_byte(0x68, 0x37, 2);
        self.i2c.write_mem_byte(0x68, 0x6B, 0);
 
         # Set the number of samples
        conf_a = (self.samples << 5) + (self.rate << 2)
        self.i2c.write_mem_byte(self.address, HMC5883L_CONF_REG_A, conf_a)

        # Set the gain
        conf_b = self.gain << 5
        self.i2c.write_mem_byte(self.address, HMC5883L_CONF_REG_B, conf_b)

        # Set the operation mode
        self.i2c.write_mem_byte(self.address, HMC5883L_MODE_REG, self.sampling_mode)
    ...
    def read(self):
        ...
        '''
        raw_data = self.i2c.read_mem_block(self.address, HMC5883L_DATA_START_BLOCK, 6)
